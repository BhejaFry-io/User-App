  ...
  return (
    <div className={`h-[100dvh] w-full ${bgClass} flex flex-col justify-between overflow-hidden relative font-sans transition-colors duration-500 p-3 md:p-6`} 
      ...
      <div className="w-full flex justify-center shrink-0 z-50 relative pt-1 md:pt-0">
        ...
        {/* Right Side: Controls (Absolutely positioned to preserve true centering of logo) */}
          ...
          {user && (
            <div className="relative">
              <button 
                ...
                className={`w-10 h-10 md:w-12 md:h-12 rounded-full border-[3px] hover:-translate-y-1 active:translate-y-1 transition-all active:shadow-none overflow-hidden ${avatarBorder}`}
              >
                <img src={user.avatarUrl} alt="Profile" className="w-full h-full object-cover" />
              </button>

              {isProfileOpen && (
                <div className={`absolute top-full right-0 mt-2 p-3 md:p-4 rounded-xl md:rounded-2xl border-[3px] md:border-4 w-40 md:w-48 flex flex-col items-center gap-3 transition-all z-50 ${dropdownBg}`}>
                  <span className={`font-black text-xs md:text-sm text-center truncate w-full ${headingText}`}>
                    {user.username}
                  </span>
                  <button 
                    onClick={() => {
        ...
        <div className={`border-[4px] md:border-[6px] p-5 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] w-full max-w-sm md:max-w-md relative transition-all duration-300 max-h-full overflow-y-auto custom-scrollbar mr-2 md:mr-4 mb-2 md:mb-4 ${cardBg}`}>
          ...
          {!user ? (
            <div className="flex flex-col items-center justify-center py-4">
              ...
              <p className={`font-bold text-[10px] md:text-sm mb-8 text-center ${pText}`}>Login to start frying those brains</p>
              
              <div className="transform scale-[1.3] md:scale-[1.5] hover:scale-[1.4] md:hover:scale-[1.6] transition-all duration-300 drop-shadow-[3px_3px_0px_#000]">
                <GoogleLogin
